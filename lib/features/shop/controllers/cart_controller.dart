...
class CartController extends GetxController {
  ...
  void addToCart(ProductModel product) {
    ...
    if (product.productType == ProductType.weight.toString()) {
      ...
      if (weight.value.isEmpty) {
        Mloaders.customToast(message: 'selectWeight'.tr);
        return;
      }
      if (product.stock < weightInKilo) {
        Mloaders.warningSnackBar(
            message: 'outOfStock'.tr, title: 'warning'.tr);
        return;
      }
      ...
      updateCart();
      Mloaders.customToast(message: 'productAdded'.tr);
    } else {
      if (productQuantityInCart.value < 1) {
        Mloaders.customToast(message: 'selectQuantity'.tr);
        return;
      }
      // Variation Selected?    <==============
      if (product.productType == ProductType.size.toString() &&
          variationController.selectedVariation.value.id.isEmpty) {
        Mloaders.customToast(message: 'selectVariation'.tr);
        return;
      }
  ...
  CartItemModel convertToCartItem(
    ...
    return CartItemModel(
      ...
      size: variationController.selectedAttributes['Size'] ?? '',
      stock: product.stock,
  ...
  Future<void> updatePriceForWeightProduct(String productId) async {
    double? pricebyWeight;
